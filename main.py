...
class BotManager:
    def __init__(self, master_token: str, database: DatabaseManager):
...
async def stop_my_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = context.application.bot_data['db']
    user_id = str(update.effective_user.id)
    bot_manager = context.application.bot_data.get('bot_manager')
...
async def restart_my_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = context.application.bot_data['db']
    user_id = str(update.effective_user.id)
...
async def delete_my_clone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = context.application.bot_data['db']
    user_id = str(update.effective_user.id)
...
• Total Reminders Created: {len(reminders) + completed}
• Active Reminders: {len(reminders)}
• Completed Reminders: {completed}

<b>🎯 This Week:</b>
• Reminders Set: Coming soon
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ADMIN_IDS = [update.effective_user.id]  # Add your Telegram ID here
...
• Reminders Today: {stats['reminders_today']}
• Completed: {stats['total_completed']}

<b>🤖 Bot Management:</b>
• Total Clones: {stats['total_clones']}
• Active Clones: {len(ACTIVE_BOTS)}
• Master Bot: 🟢 Running

<b>💾 Database:</b>
• Status: Connected
• Type: MongoDB
• Host: {MONGODB_URI.split('@')[-1] if '@' in MONGODB_URI else 'localhost'}
...
<b>⏰ Reminder Analytics:</b>
• Active: {stats['total_reminders']}
• Today: {stats['reminders_today']}
• Completed: {stats['total_completed']}
• Success Rate: {(stats['total_completed'] / (stats['total_completed'] + stats['total_reminders']) * 100) if (stats['total_completed'] + stats['total_reminders']) > 0 else 0:.1f}%

<b>🤖 Clone Analytics:</b>
• Total Clones: {stats['total_clones']}
• Active Now: {len(ACTIVE_BOTS)}
• Activation Rate: {(len(ACTIVE_BOTS) / stats['total_clones'] * 100) if stats['total_clones'] > 0 else 0:.1f}%
...
/delete ID - Delete specific reminder
/stats - Your statistics

<b>🤖 Clone Bot Commands:</b>
/clone - Clone bot guide
/settoken TOKEN - Set bot token
/mystop - Stop your bot
/mystart - Restart your bot
/mydelete - Delete your bot
...
<b>💡 Pro Tips:</b>
• Use quick buttons for common times
• Clone bot runs 24/7 automatically
• All data stored in secure MongoDB
• Multiple reminders supported

<b>🎯 Examples:</b>
/remind Drink water @30m
/remind Gym workout @2h
/remind Pay bills @1d
/remind Weekly meeting @7d

<b>❓ Need Help?</b>
Contact support or use inline buttons!
    """
...
<b>Examples:</b>
<code>/remind Drink water @30m</code>
<code>/remind Exercise @2h</code>
<code>/remind Call mom @1d</code>
<code>/remind Weekly review @7d</code>

<b>💡 Tips:</b>
• Keep messages clear and short
• Use multiple reminders for tasks
• Check /list to see all active
...
3️⃣ Follow instructions to create bot
4️⃣ Copy the token you receive
5️⃣ Send here: <code>/settoken TOKEN</code>
...
Ready to create? Use /clone command!
        """
        keyboard = [[InlineKeyboardButton("📖 Detailed Guide", url="https://t.me/BotFather")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
✅ Completed: {completed}
🤖 Clone Bot: {'Yes ✅' if stats['has_clone'] else 'No ❌'}
